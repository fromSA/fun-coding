...
applyUpdateRuleOnCell (Cell s p) ps = case s of
                                        Alive -> if nr_alive >= 4 then Cell Dead p else Cell Alive p
                                        Dead -> if nr_dead >= 4 then Cell Alive p else Cell Dead p
                                        where
                                            nr_alive = length $ filter (\(Cell s p) -> s == Alive) ps
                                            nr_dead = length $ filter (\(Cell s p) -> s == Dead) ps
